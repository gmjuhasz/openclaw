// Messenger plugin module implements monitor (webhook inbound) behavior.
...
export async function monitorMessengerProvider(
  ...
  const unregisterHttp = registerPluginHttpRoute({
    ...
    handler: async (req: IncomingMessage, res: ServerResponse) => {
      // Handle GET requests for webhook verification (Meta hub-challenge handshake)
      if (req.method === "GET") {
  ...
  // Webhook providers have no long-lived socket; block until the gateway aborts
  // so the channel stays "running" instead of exiting immediately after the
  // route is registered (mirrors the LINE provider lifecycle).
